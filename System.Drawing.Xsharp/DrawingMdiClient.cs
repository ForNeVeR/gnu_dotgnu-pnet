/*
 * DrawingMdiClient.cs - Implementation of MDI clients for System.Drawing.
 *
...
internal sealed class DrawingMdiClient : MdiClientWidget, IToolkitMdiClient
{
	// Internal state.
	...
	void IToolkitMdiClient.Cascade()
			{
				Cascade();
			}

	// Tile the child windows horizontally.
	void IToolkitMdiClient.TileHorizontally()
			{
				TileHorizontally();
			}

	// Tile the child windows vertically.
	void IToolkitMdiClient.TileVertically()
			{
				TileVertically();
			}

	// Arrange the icons in the client area.
	void IToolkitMdiClient.ArrangeIcons()
			{
				ArrangeIcons();
			}
