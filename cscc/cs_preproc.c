/*
 * cs_preproc.c - Pre-processor for C# source code.
 *
...
void CSPreProcDestroy(CSPreProc *preproc)
{
	...
	CSPreProcSymbol *next;
	CSPreProcScope *scope;
	CSPreProcScope *nextScope;
...
void CSPreProcDefine(CSPreProc *preproc, const char *name)
{
	...
	if((symbol = (CSPreProcSymbol *)ILMalloc(sizeof(CSPreProcSymbol) +
											 strlen(name))) == 0)
	{
...
{
	CSPreProcSymbol *symbol = preproc->symbols;
	while(symbol != 0)
	{
		if(!strcmp(symbol->name, name))
		{
 ...
 */
static int AddLine(CSPreProc *preproc, int posn, int len, int directive)
{
	CSPreProcLine *newLines;
	if(preproc->numLines >= preproc->maxLines)
	{
		newLines = (CSPreProcLine *)ILRealloc
			(preproc->lines, sizeof(CSPreProcLine) * (preproc->maxLines + 8));
		if(!newLines)
		{
			return 0;
		}
 ...
 */
static int GetLines(CSPreProc *preproc)
{
 ...
 */
static char *ParseIdentifier(CSPreProc *preproc, CSPreProcLine *info,
							 char **_line)
{
	char *line;
	int len;
	...
	{
		Message(info, "invalid pre-processor identifier: ", identifier);
		preproc->error = 1;
		return 0;
	}
 ...
 */
static void NextToken(char **_line, CSPreProcToken *token)
{
	char *line;
	int len;
 ...
 */
static int ParsePrimaryExpression(CSPreProc *preproc, char **line,
								  CSPreProcToken *token)
{
	int result;
	...
	{
		result = CSPreProcIsDefined(preproc, token->name);
		NextToken(line, token);
		return result;
	}
 ...
 */
static int ParseUnaryExpression(CSPreProc *preproc, char **line,
								CSPreProcToken *token)
{
 ...
 */
static int ParseExpression(CSPreProc *preproc, CSPreProcLine *info, char **line)
{
	CSPreProcToken token;
	int result;
	...
	{
		Message(info, "invalid pre-processor expression", 0);
		preproc->error = 1;
		result = 0;
	}
 ...
 */
static void CheckAtEnd(CSPreProc *preproc, CSPreProcLine *info, char **line)
{
	SkipWhite(line);
	if((*(*line)) != '\n')
	{
		Message(info, "spurious data on pre-processor directive line", 0);
		preproc->error = 1;
	}
}
 ...
 */
static int RefillLineBuffer(CSPreProc *preproc)
{
	CSPreProcLine *lines;
	int line, cond;
	char *dirname;
	...
	if(!GetLines(preproc))
	{
		if(!(preproc->reportedUnmatched))
		{
			...
			while(scope != 0)
			{
				if(scope->filename)
				{
					fprintf(stderr, "%s:%lu: #if without matching #endif\n",
							scope->filename, scope->number);
				}
				else
				{
					fprintf(stderr, "%lu: #if without matching #endif\n",
							scope->number);
				}
				preproc->error = 1;
				scope = scope->next;
	...
	for(line = 0; line < preproc->numLines; ++line)
	{
		   ...
		   (dirname = DirectiveName(preproc->buffer + lines[line].posn)) != 0)
		{
			...
			else if(MATCH_DIRECTIVE(dirname, "if", 2))
			{
				...
				scope = (CSPreProcScope *)ILMalloc(sizeof(CSPreProcScope));
				if(scope)
				{
					scope->active = cond;
					scope->previous = 0;
			...
			else if(MATCH_DIRECTIVE(dirname, "elif", 4))
			{
				...
				if(preproc->currentScope)
				{
					...
					{
						Message(&(lines[line]), "#elif used after #else", 0);
						preproc->error = 1;
					}
				}
				else
				{
					Message(&(lines[line]), "unmatched #elif", 0);
					preproc->error = 1;
				}
			}
			else if(MATCH_DIRECTIVE(dirname, "else", 4))
			{
				...
				if(preproc->currentScope)
				{
					...
					if(preproc->currentScope->sawElse)
					{
						Message(&(lines[line]), "#else used after #else", 0);
						preproc->error = 1;
					}
				...
				{
					Message(&(lines[line]), "unmatched #else", 0);
					preproc->error = 1;
				}
			}
			else if(MATCH_DIRECTIVE(dirname, "endif", 5))
			{
				...
				if(preproc->currentScope)
				{
					CSPreProcScope *next = preproc->currentScope->next;
					ILFree(preproc->currentScope);
					preproc->currentScope = next;
				}
				else
				{
					Message(&(lines[line]), "unmatched #endif", 0);
					preproc->error = 1;
				}
			}
			...
			else if(MATCH_DIRECTIVE(dirname, "line", 4))
			{
				...
				if(*dirname >= '0' && *dirname <= '9')
				{
					...
					if(*dirname == '"')
					{
						...
						{
							Message(&(lines[line]),
									"badly formatted #line directive", 0);
							preproc->error = 1;
						}
						else
						{
			...
			else
			{
				/* Unknown directive */
				preproc->buffer[lines[line].posn + lines[line].len - 1] = '\0';
				Message(&(lines[line]), "unknown pre-processor directive: ",
