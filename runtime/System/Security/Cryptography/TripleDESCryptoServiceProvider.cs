/*
 * TripleDESCryptoServiceProvider.cs - Implementation of the
...
public sealed class TripleDESCryptoServiceProvider : TripleDES
{
	...
	public TripleDESCryptoServiceProvider()
			: base()
			{
				if(!CryptoMethods.AlgorithmSupported(CryptoMethods.TripleDES))
				{
					throw new CryptographicException
						(_("Crypto_NoProvider"), "TripleDES");
				}
			}

	// Create a TripleDES decryptor object.
	public override ICryptoTransform CreateDecryptor
				(byte[] rgbKey, byte[] rgbIV)
			{
				ValidateCreate(rgbKey, rgbIV);
				return new CryptoAPITransform
						(CryptoMethods.TripleDES, rgbIV, rgbKey,
						 BlockSizeValue, FeedbackSizeValue,
						 ModeValue, PaddingValue, false);
			}

	// Create a TripleDES encryptor object.
